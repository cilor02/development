...
public class TransformShape{
	...
	private Shape upperQuadrant;
	public TransformShape( int unit, int height, int width, double startX, double startY) {
        this.height = height;
        this.width = width;
		this.unit= unit;
		startPoint = new Point2D.Double(startX,startY);
		int[]xints = {0,width/2,width/2,0};
		int[]yints = {0,0,height/2,height/2};
		points = new ArrayList<Point2D>();
		upperQuadrant = new java.awt.Polygon(xints,yints,4);
		derivePoints();
	}
	...
	private void derivePoints ()
	{
		...
		for(int i = 0 ; i < (moves + 4);i++)
		{
			...
			}while (!upperQuadrant.contains(currentPoint));
